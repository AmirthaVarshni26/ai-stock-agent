...
import { TrendingUp, MessageSquare, Zap, ArrowUpRight, Search } from "lucide-react";
// 1. Import your custom hook
import { useStockData } from '@/hooks/useStockData';

export default function StockDashboard() {
  // 2. Call the hook for Bitcoin (Active 24/7)
    ...
    <div className="min-h-screen bg-black text-white flex flex-col font-sans selection:bg-yellow-500/30">
      ...
      <header className="border-b border-zinc-800 p-4 flex justify-between items-center bg-zinc-900/30 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-zinc-700 to-zinc-900 border border-zinc-700 flex items-center justify-center font-bold text-xs uppercase">
            AV
          </div>
        </div>
      </header>
             ...
             <div className="flex justify-between items-start mb-10 relative z-10">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="bg-white/10 px-2 py-1 rounded text-[10px] font-bold tracking-widest uppercase">Crypto</span>
                    <h2 className="text-5xl font-black tracking-tighter">BTC</h2>
                  </div>
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-3xl flex flex-col h-full overflow-hidden backdrop-blur-sm">
            <div className="p-5 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/40">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[8px] text-zinc-500 font-bold">MODE: REAL-TIME RAG</p>
                </div>
              </div>
              <div className="px-2 py-1 rounded-md bg-white/5 border border-white/10 text-[9px] font-black tracking-tighter">
                LLAMA-3-GROQ
              </div>
            </div>
            
            {/* AI Chat History */}
            <div className="flex-1 p-6 text-sm space-y-6 overflow-y-auto">
              <div className="space-y-2">
                <div className="bg-zinc-800/40 p-4 rounded-2xl rounded-tl-none border border-zinc-700/50 leading-relaxed text-zinc-300">
                    <p className="text-[10px] font-black text-yellow-500 mb-2 uppercase tracking-widest underline decoration-2 underline-offset-4">Insights: BTC</p>
                    Greetings, Amirtha. I am monitoring the Bitcoin volatility. Current liquidity levels suggest a strong consolidation phase. Momentum remains <span className="text-green-400 font-bold">BULLISH</span>.
                </div>
                <p className="text-[9px] text-zinc-600 font-medium px-1">Active Monitoring • Confidence 94%</p>
              </div>
            </div>
            ...
            <div className="p-6 bg-zinc-900/40 border-t border-zinc-800">
              <div className="relative group">
                ...
                <input 
                    type="text" 
                    placeholder="Search ticker or ask Analyst..." 
                    className="w-full bg-black border border-zinc-800 rounded-2xl p-4 pl-12 text-sm focus:outline-none focus:border-yellow-500/50 transition-all placeholder:text-zinc-600 relative z-10"
                />
